...
insert into test2(filepath, bno) values('c:/downlaod/x.gif', 100);
...
- 그래서 데이터의 무결성(Data integrity)을 유지하게 도와준다
...
위와 같이 test2 테이블에 FK제약 조건을 건 다음에 데이터를 입력해보자!
...
/*5번, 10번 게시물은 존재하기 때문에 데이터를 입력할 수 있다*/
...
insert into test2(filepath, bno) values('c:/download/g.gif',10);

/*5번 게시물을 삭제할 수 없다. 왜 ? test2 테이블의 데이터 중 일부가 참조하기 때문에다. */
delete from test1 where no=2;
delete from test1 where no=5; --error

##용어 정리 
- test1처럼 다른 테이블에 의해 참조되는 테이블을 '부모테이블' 이라 부른다.
