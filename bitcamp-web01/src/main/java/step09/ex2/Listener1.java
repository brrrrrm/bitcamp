...
//=> 특정 상태 도달에 걸리는 시간 등의 정보를 기록하여 서버의 실행 상태를 보여주는 모니터링 시스템을 만들 때 유용하다
//예) 애플리케이션 성능 모니터링 시스템 (Application Performance Monitoring System : APM)
                                                                                        
//서블릿 컨테이너에서 발생하는 상태(event, 사건, 상태)
//1)서블릿 컨테이너의 시작/종료 이벤트
//2)요청의 시작/종료 이벤트
//3)ServletRequest에 값을 저장하고 제거하는 이벤트
//4)HttpSession 객체가 생성되고 제거되는 이벤트
//5)HttpSession 에 값을 저장하고 제거하는 이벤트

//리스너 만들기
//=> 이벤트가 발생했을 때 어떤 메소드를 호출할지 Servlet 기술에 명세되어 있다
//=> 특정 이벤트가 발생했을 때 어떤 작업을 처리하고 싶다면 
//   그 이벤트에 대한 규칙에 따라 클래스를 정의한다

//예 : 서블릿 컨테이너가 시작되거나 종료될 때 그 시간을 출력하고 싶다
//서블릿 컨테이너가 시작되거나 종료될 때 호출하는 메소드를 정의한 규칙을 구현한다

//리스너 배치
//서블릿 컨테이너에게 이벤트를 처리할 리스너를 등록해야만 동작한다
//배치하는 방법
//1)애노테이션을 사용하여 배치 @WebListener
...
public class Listener1 implements ServletContextListener{
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        // 여기에는 모든 서블릿들이 사용할 도구를 준비하는 코드를 두면 된다 
    ...
    public void contextDestroyed(ServletContextEvent sce) {
        ...
        // 서블릿 컨테이너가 시작될 때 준비한 도구나 자원을 해제하는 코드를 둔다
