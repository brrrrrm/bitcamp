...
[JSP 코드]
 &lt;%@ taglib file="태그 라이브러리 이름" prefix="별명" %>

태그 라이브러리 이름
 - JSP 확장 태그가 정의된 라이브러리 이름이다.
 - 보통 URL 형태로 되어 있다.
 - 라이브러리를 만든 사람이 정한 대로 지정해야 한다.
 
 별명 
 - 라이브러리에 들어 있는 태그를 사용할 때 앞에 붙이는 접두어이다. 
 - JSP 기본 라이브러리나 유명 라이브러리를 사용할 때는 가능한 다른 개발자들이 관습적으로 사용하는 별명을 지정하라
...
1)mvnrepository.com에서 "jstl"라이브러리 검색. 
