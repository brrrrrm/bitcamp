...
<%
//jsp에서 저장소 객체를 사용하려면 pageContext, request, session, application 이름으로 된 객체를 사용해야한다
...
ServletRequest : ${applicationScope.name} <br>
ServletRequest: <%=application.getAttribute("name") %> <br>
...
위와 같은 이유로 getAttribute() 메소드를 호출하는 EL 표기법을 만들기 위해 xxxScope이라는 이름을 만든 것이다. 
